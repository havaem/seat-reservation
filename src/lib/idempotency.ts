...
export const Idempotency = models.Idempotency || model("Idempotency", IdemSchema);

export async function withIdempotency(req: Request, handler: () => Promise<Response>) {
  ...
  try {
    ...
    await Idempotency.create({ key, method: req.method, path: new URL(req.url).pathname, response: json });
  } catch {}
  return res;
}